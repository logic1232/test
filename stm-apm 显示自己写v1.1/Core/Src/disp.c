    ...
    clear_screen();
	 ...
	 Lcd12864_Write16CnCHAR(0,64,2,"Ia:   A\r");
		 ...
		 Lcd12864_Write16CnCHAR(0,64,6,"Ic:   A\r");
			...
			xianshishuju(nor_disp_num_buf[4]);          	//电流数据
...
//				key_state=0x1100;
    ...
    if((key_state>=0x12100)&&(key_state<=0x12107))//额定漏电
    {
...
void Get_record(unsigned int tmp1)
{
		...
		if(point_tmp<=9)
		{
			EEPROM_R((10*point_tmp),record_array,10,0xA0);
...
//					if((L_factor_tmp1>-20)&&(L_factor_tmp1<20))//漏电手动校准
					{       
      ...
      if(key==0x80)//向下键
			 {
					         ...
					         tiaozheng_key_state(0x11100,17,1);         //漏电告警：   0：关，1：开
             if(D_LCur!=8)
						 {
...
//					 tiaozheng_key_state(0x75100,2,1);
					...
					tiaozheng_key_state(0x50100,10,0);		
				}
			 ...
			 if(key==0x01)//向上键
			 {
					 	tiaozheng_key_state(0x11100,15,0);         //漏电告警：   0：关，1：开
...
//					 tiaozheng_key_state(0x39100,2,0);               //电流重合闸
           ...
           tiaozheng_key_state(0x2A100,5,1);    //壳架等级
					 ...
					 tiaozheng_key_state(0x50100,10,1);		
				}
//////////	     //返回
//////////       if(key==0x40)
//////////        {
...
//	}
		disp_data_prepare(); 
		if((F_sssj==0)&&(F_gz_disp==0x0f))//无故障
		{
