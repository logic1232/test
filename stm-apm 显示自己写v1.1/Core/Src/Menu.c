...
unsigned char  qirigang[16]={0x00,0x00,0xC0,0x38,0x06,0x00,0x00,0x00,0x30,0x0E,0x01,0x00,0x00,0x00,0x00,0x00,};
...
}else if(key==KEY_RIGHRT_PRES&&*(menu[a].value)>0)
{
	...
	{
		case 0: if(a<totalItems)
		{


				display_ASCIIstring_8x16_fanxian(1, 93, menu[0].A[*(menu[0].value)]);
				if(totalItems>1)display_ASCIIstring_8x16(3, 93, menu[1].A[*(menu[1].value)] );
				if(totalItems>2)display_ASCIIstring_8x16(5, 93, menu[2].A[*(menu[2].value)]);
				if(totalItems>3)display_ASCIIstring_8x16(7, 93, menu[3].A[*(menu[3].value)]);
	}
					
		break;
        case 1: if(a<totalItems){
	
			display_ASCIIstring_8x16(1, 93, menu[0].A[*(menu[0].value)]);
			if(totalItems>1)display_ASCIIstring_8x16_fanxian(3, 93, menu[1].A[*(menu[1].value)] );
			if(totalItems>2)display_ASCIIstring_8x16(5, 93, menu[2].A[*(menu[2].value)]);
			if(totalItems>3)display_ASCIIstring_8x16(7, 93, menu[3].A[*(menu[3].value)]);}
		break;
	
        case 2: if(a<totalItems){

			display_ASCIIstring_8x16(1, 93, menu[0].A[*(menu[0].value)]);
			if(totalItems>1)display_ASCIIstring_8x16(3, 93, menu[1].A[*(menu[1].value)] );
			if(totalItems>2)display_ASCIIstring_8x16_fanxian(5, 93, menu[2].A[*(menu[2].value)]);
			if(totalItems>3)display_ASCIIstring_8x16(7, 93, menu[3].A[*(menu[3].value)]);}
		break;
		case 3:if(a<totalItems){ 

			display_ASCIIstring_8x16(1, 93, menu[0].A[*(menu[0].value)]);
			if(totalItems>1)display_ASCIIstring_8x16(3, 93, menu[1].A[*(menu[1].value)] );
			if(totalItems>2)display_ASCIIstring_8x16(5, 93, menu[2].A[*(menu[2].value)]);
			if(totalItems>3)display_ASCIIstring_8x16_fanxian(7, 93, menu[3].A[*(menu[3].value)]);}
		break;
...
{
	if(Type_Cur==0)
{ D_Cur_P=8;for (int i = 0; i < 9; ++i) { strcpy((char *)guoliu_show[i], (char *)guoliu_type1[i]);}}
	if(Type_Cur==1)
{ D_Cur_P=19;for (int i = 0; i < 20; ++i) { strcpy((char *)guoliu_show[i], (char *)guoliu_type2[i]);}}
	if(Type_Cur==2)
{ D_Cur_P=12;for (int i = 0; i < 13; ++i) { strcpy((char *)guoliu_show[i], (char *)guoliu_type3[i]);}}
	if(Type_Cur==3)
{ D_Cur_P=13;for (int i = 0; i < 14; ++i) { strcpy((char *)guoliu_show[i], (char *)guoliu_type4[i]);}}
	if(Type_Cur==4)
{ D_Cur_P=7;for (int i = 0; i < 7; ++i) { strcpy((char *)guoliu_show[i], (char *)guoliu_type5[i]);}}
...
void showScreen(int screenIndex) {     //轮询的展示实现
    ...
    switch (screenIndex) {
		...
		case 6:		screenbase7();
            ...
            break;
			case 10:		screenbase11();
					
            break;
			case 11:		screenbase12();
					
            break;
			case 12:		screenbase13();
...
void screenbase1()	//轮询1界面
	{
			switch (HT7038_buf1[46]){
				case 0: Lcd12864_Write16CnCHAR(0,5,0,"↗");
			break;
			case 1: Lcd12864_Write16CnCHAR(0,5,0,"↗");
			break;
			case 2: Lcd12864_Write16CnCHAR(0,5,0,"↖");
			break;
			case 3: Lcd12864_Write16CnCHAR(0,5,0,"↙");
			break;
			case 4: Lcd12864_Write16CnCHAR(0,5,0,"↘");
			break;
			}
...
{        
	...
	switch (a1)
	{
		case 0: F_MAINormenu=2;   
		break;
		case 1: F_MAINormenu=1;   
			break;
		case 2: F_MAINormenu=5;   //电能参数
			break;
		case 3: {USART_LCD_Transmit(1,6);
			
			USART_LCD_Transmit(1,6);
			F_MAINormenu=8;   } //电力品质
			break;
		case 4: F_MAINormenu=9;    //需量查看
			break;
		case 5: 
...
{        
	...
	switch (a)
	{
		case 0: F_AEorREorALLE=1;   
		break;
		case 1: F_AEorREorALLE=2;   
			break;
		case 2: F_AEorREorALLE=3;   
			break;
...
void power_quality()//电力品质
{
	...
	if(current_page==0&&F_Dataval==0&&f_f!=0)
	{
		...
		switch (f_f)
		{
			case 1:disp_data(40,5,14,0);break;
			case 2:disp_data(40,5,14,166);break;
			case 3:disp_data(40,5,14,333);break;
			case 4:disp_data(40,5,14,500);break;
			case 5:disp_data(40,5,14,667);break;
			case 6:disp_data(40,5,14,833 );break;
	}
...
}else if(key==KEY_RIGHRT_PRES&&(*(meunprotect[a].openorclose) )==1)
{
		...
		switch (a)  //a是按了几次 d是判断出来的有几行    //这个switch是选择是哪一行
	{
		...
		display_kaiguan_frame(1, 106, *(meunprotect[0].openorclose));
				if(totalItems>1)display_kaiguan(3, 106, *(meunprotect[1].openorclose));
				if(totalItems>2)display_kaiguan(5, 106, *(meunprotect[2].openorclose));
				if(totalItems>3)display_kaiguan(7, 106, *(meunprotect[3].openorclose));
	}
        ...
        case 1: if(a<totalItems){
			...
			if(totalItems>1)display_kaiguan_frame(3, 106, *(meunprotect[1].openorclose) );
			if(totalItems>2)display_kaiguan(5, 106, *(meunprotect[2].openorclose));
			if(totalItems>3)display_kaiguan(7, 106, *(meunprotect[3].openorclose));}
		break;
        ...
        case 2: if(a<totalItems){
			...
			display_kaiguan(1, 106, *(meunprotect[0].openorclose));
			if(totalItems>1)display_kaiguan(3, 106, *(meunprotect[1].openorclose) );
			if(totalItems>2)display_kaiguan_frame(5, 106,*(meunprotect[2].openorclose));
			if(totalItems>3)display_kaiguan(7, 106, *(meunprotect[3].openorclose));}
		break;
		case 3:if(a<totalItems){ 
			...
			if(totalItems>1)display_kaiguan(3, 106, *(meunprotect[1].openorclose) );
			if(totalItems>2)display_kaiguan(5, 106,  *(meunprotect[2].openorclose));
			if(totalItems>3)display_kaiguan_frame(7, 106, *(meunprotect[3].openorclose));}
		break;
...
//	if(a>totalItems-1&&F_Ypro!=4)
	...
	a=0;
			clear_screen();
		if(F_Ypro==3) F_Ypro=4;
		  if(F_Ypro==2) F_Ypro=3;
		  if(F_Ypro==1) F_Ypro=2;
		  if(F_Ypro==0) F_Ypro=1;
	}
//	}else if(a>totalItems-1&&F_Y==4) a=totalItems-1;
}
else if(key==KEY_DOWN_PRES)
{
	...
	if(a<0)
	{
		a=0;
		clear_screen();
	if(F_Ypro==1) F_Ypro=0;
		if(F_Ypro==2) F_Ypro=1;	
		if(F_Ypro==3) F_Ypro=2;
		if(F_Ypro==4) F_Ypro=3;
	}	
}else if(key==KEY_AFFIRM_PRES)    //确认之后要加数据比较，有变更发送数据
{        
	judge();
...
void meun_protect1()  //保护界面1
{
	...
	if(F_trip_ld==0)ctrlword[3]&=0xFC ;//控制字分析用于发送
  if(F_trip_ld==1)
    {
     ctrlword[3]&=0xFC ;
     ctrlword[3]+=0x02 ;
...
{        
		switch (current_page){
		case 0: F_MAINormenu=12;
		break;
		case 1:F_MAINormenu=13;
...
void displayMenuadj(meunadj meunadjust[],int total_items)
{
	 ...
	 if(F_jiaozhuned==1)
	{
		...
		if(Type_Cur==0)Lcd12864_Write16CnCHAR(0,24,0,meunadjust[2].NAME1);
		if(Type_Cur==1)Lcd12864_Write16CnCHAR(0,24,0,meunadjust[3].NAME1);
		if(Type_Cur==2)Lcd12864_Write16CnCHAR(0,24,0,meunadjust[4].NAME1);
		if(Type_Cur==3)Lcd12864_Write16CnCHAR(0,24,0,meunadjust[5].NAME1);
		if(Type_Cur==4)Lcd12864_Write16CnCHAR(0,24,0,meunadjust[6].NAME1);
...
RTC_Set_Date(tim_y3,tim_m3,tim_d3);
		...
		clear_screen();
			switch (a){
			case 0:{tim_y3++;if((tim_y3&0x0F)>9)tim_y3+=6;}
			break;
			case 1:{if(tim_m3<18){ tim_m3++;if((tim_m3&0x0F)>9)tim_m3+=6;}}
			break;
			case 2:{if(tim_d3<49){ tim_d3++;if((tim_d3&0x0F)>9)tim_d3+=6;}}
			break;			
			case 3:{if(tim_h3<36){ tim_h3++;if((tim_h3&0x0F)>9)tim_h3+=6;}}
			break;
			case 4:{if(tim_f3<96){ tim_f3++;if((tim_f3&0x0F)>9)tim_f3+=6;}}
			break;
			case 5:{if(tim_s3<96){ tim_s3++;if((tim_s3&0x0F)>9)tim_s3+=6;}}
			break;
			}		
		}else if(key==KEY_RIGHRT_PRES){	
		clear_screen();
			switch (a){
			case 0:{if(tim_y3>0){tim_y3--;if((tim_y3&0x0F)==15)tim_y3-=6;}}
			break;
			case 1:{if(tim_m3>0){tim_m3--;if((tim_m3&0x0F)==15)tim_m3-=6;}}
			break;
			case 2:{if(tim_d3>0){tim_d3--;if((tim_d3&0x0F)==15)tim_d3-=6;}}
			break;			
			case 3:{if(tim_h3>0){tim_h3--;if((tim_h3&0x0F)==15)tim_h3-=6;}}
			break;
			case 4:{if(tim_f3>0){tim_f3--;if((tim_f3&0x0F)==15)tim_f3-=6;}}
			break;
			case 5:{if(tim_s3>0){tim_s3--;if((tim_s3&0x0F)==15)tim_s3-=6;}}
			break;
...
switch (a)
{
	...
	a++;
		...
		clear_screen();
			switch (a){
			case 0:{if(mima_set_compare[0]<9)mima_set_compare[0]++;}
			break;
			case 1:{ if(mima_set_compare[1]<9)mima_set_compare[1]++ ;}
			break;
			case 2:{if(mima_set_compare[2]<9)mima_set_compare[2]++;}
			break;			
			case 3:{if(mima_set_compare[3]<9)mima_set_compare[3]++;}
			break;
			case 4:{if(self_addr_code[5]>>4<9)self_addr_code[5]=self_addr_code[5]+16;}
			break;
			case 5:{if((self_addr_code[5]&0x0F)<9)self_addr_code[5]=self_addr_code[5]+1;}
			break;
			case 6:{if(self_addr_code[4]>>4<9)self_addr_code[4]=self_addr_code[4]+16;}
			break;
			case 7:{if((self_addr_code[4]&0x0F)<9)self_addr_code[4]=self_addr_code[4]+1;}
			break;
			case 8:{if(self_addr_code[3]>>4<9)self_addr_code[3]=self_addr_code[3]+16;}
			break;
			case 9:{if((self_addr_code[3]&0x0F)<9)self_addr_code[3]=self_addr_code[3]+1;}
			break;
			case 10:{if(self_addr_code[2]>>4<9)self_addr_code[2]=self_addr_code[2]+16;}
			break;
			case 11:{if((self_addr_code[2]&0x0F)<9)self_addr_code[2]=self_addr_code[2]+1;}
			break;
			case 12:{if(self_addr_code[1]>>4<9)self_addr_code[1]=self_addr_code[1]+16;}
			break;
			case 13:{if((self_addr_code[1]&0x0F)<9)self_addr_code[1]=self_addr_code[1]+1;}
			break;
			case 14:{if(self_addr_code[0]>>4<9)self_addr_code[0]=self_addr_code[0]+16;}
			break;
			case 15:{if((self_addr_code[0]&0x0F)<9)self_addr_code[0]=self_addr_code[0]+1;}
			break;
			}		
		}else if(key==KEY_RIGHRT_PRES){	
		clear_screen();
			switch (a){
			case 0:{if(mima_set_compare[0]>0)mima_set_compare[0]--;}
			break;
			case 1:{if(mima_set_compare[1]>0) mima_set_compare[1]-- ;}
			break;
			case 2:{if(mima_set_compare[2]>0)mima_set_compare[2]--;}
			break;			
			case 3:{if(mima_set_compare[3]>0)mima_set_compare[3]--;}
			break;
			case 4:{if(self_addr_code[5]>>4>0)self_addr_code[5]=self_addr_code[5]-16;}
			break;
			case 5:{if((self_addr_code[5]&0x0F)>0)self_addr_code[5]=self_addr_code[5]-1;}
			break;
			case 6:{if(self_addr_code[4]>>4>0)self_addr_code[4]=self_addr_code[4]-16;}
			break;
			case 7:{if((self_addr_code[4]&0x0F)>0)self_addr_code[4]=self_addr_code[4]-1;}
			break;
			case 8:{if(self_addr_code[3]>>4>0)self_addr_code[3]=self_addr_code[3]-16;}
			break;
			case 9:{if((self_addr_code[3]&0x0F)>0)self_addr_code[3]=self_addr_code[3]-1;}
			break;
			case 10:{if(self_addr_code[2]>>4>0)self_addr_code[2]=self_addr_code[2]-16;}
			break;
			case 11:{if((self_addr_code[2]&0x0F)>0)self_addr_code[2]=self_addr_code[2]-1;}
			break;
			case 12:{if(self_addr_code[1]>>4>0)self_addr_code[1]=self_addr_code[1]-16;}
			break;
			case 13:{if((self_addr_code[1]&0x0F)>0)self_addr_code[1]=self_addr_code[1]-1;}
			break;
			case 14:{if(self_addr_code[0]>>4>0)self_addr_code[0]=self_addr_code[0]-16;}
			break;
			case 15:{if((self_addr_code[0]&0x0F)>0)self_addr_code[0]=self_addr_code[0]-1;}
			break;
...
array_tmp[27] = ( TH_TEM_UP >> 8) & 0xFF;
		...
		case 2: //漏电分闸合闸
			DI3 = 0x32;DI2 = 0x01;DI0 = 0x03;
				switch(F_LEAK_OPEN_CLOSE)   //F_LEAK_OPEN_CLOSE  1是漏电2是分闸3是合闸          F_open==1是高温分闸,F_open==0是手动分闸
				{
					case 1:array_tmp[0]=1;array_tmp[1]=0;array_tmp[2]=0;array_tmp[3]=0;array_tmp[4]=0;array_tmp[5]=0;array_tmp[6]=0; break;
					case 2:if(F_open==1){array_tmp[0]=0;array_tmp[1]=0;array_tmp[2]=0;array_tmp[3]=1;array_tmp[4]=tem;array_tmp[5]=max_value;array_tmp[6]=0;}    //还没改，温度处理那，现在写死的warray_tmp[4]=2;array_tmp[5]=122   
						else if(F_open==0){array_tmp[0]=0;array_tmp[1]=1;array_tmp[2]=0;array_tmp[3]=0;array_tmp[4]=0;array_tmp[5]=0;array_tmp[6]=0;} break;  //手动分闸
					case 3:array_tmp[0]=0;array_tmp[1]=0;array_tmp[2]=1;array_tmp[3]=0;array_tmp[4]=1;array_tmp[5]=0;array_tmp[6]=0; break;         //手动合闸
					case 4	:array_tmp[0]=0;array_tmp[1]=0;array_tmp[2]=1;array_tmp[3]=0;array_tmp[4]=0;array_tmp[5]=0;array_tmp[6]=1; break;         //低温合闸
				}
		Write_or_Readresponse(0x10,len,array_tmp); 
			break;
		case 3: //校准相关
...
Lcd12864_Write16CnCHAR(0,0,2,"类型:");
	...
	Lcd12864_Write16CnCHAR(0,40,2,guzhang_yuanyin[fault_buffer[min_index+current_page].fault_type]);
		...
		{
			case 0: Lcd12864_Write16CnCHAR(0,110,4,"V"); break ;
			case 1:	Lcd12864_Write16CnCHAR(0,110,4,"V"); break ;
			case 2:	Lcd12864_Write16CnCHAR(0,110,4,"mA");break ;
			case 3:	Lcd12864_Write16CnCHAR(0,110,4,"A"); break ;
			case 4:Lcd12864_Write16CnCHAR(0,110, 4," ");  break ;
			case 5:Lcd12864_Write16CnCHAR(0,110, 4," ");  break ;
			case 6:Lcd12864_Write16CnCHAR(0,110, 4," ");  break ;
			case 7:Lcd12864_Write16CnCHAR(0,110, 4," ");  break ;
			case 8:Lcd12864_Write16CnCHAR(0,110, 4," ");  break ;
			case 9:Lcd12864_Write16CnCHAR(0,110, 4,"V");  break ;
			case 10:Lcd12864_Write16CnCHAR(0,110,4,"V"); break ;
			case 11:Lcd12864_Write16CnCHAR(0,110,4,"A"); break ;
			case 12:Lcd12864_Write16CnCHAR(0,110,4,"A"); break ;
	...
	else Lcd12864_Write16CnCHAR(0,20,3,"无故障信息");
	if(key==KEY_DOWN_PRES)
{
	 if (current_page > 0) {
		 clear_screen();
...
int parse_voltage(const unsigned char *voltage_str,char a) {
	...
	switch(a){
		case 0:buffer[strlen(buffer) - 0] = '\0';  break;
		case 1:buffer[strlen(buffer) - 1] = '\0';  break;
		case 2:buffer[strlen(buffer) - 2] = '\0';  break;
		case 3:buffer[strlen(buffer) - 3] = '\0';  break;
		case 4:buffer[strlen(buffer) - 4] = '\0';  break;
	}
