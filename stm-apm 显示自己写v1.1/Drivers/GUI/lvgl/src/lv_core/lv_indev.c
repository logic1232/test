...
//extern bool cover_state;  //封面页标志：1  
//extern bool cover_state1; //封面页(三个功率）标志：1 
//extern bool cover_state2; //封面页(温度）标志：1 
//extern bool detail_state;  //详细页标志：1  
//extern bool data_state;  //数据设置标志：1  
//extern bool pro_state;  //保护类型设置标志：1 
/*********************
 *      DEFINES
 *********************/
 
extern lv_obj_t * focused_btn; //被聚焦的按钮
...
//					focused_btn=lv_group_get_focused(g);//获取当前被聚焦的对象
//					lv_list_focus(focused_btn, LV_ANIM_OFF); //使其显示，动画关闭
//						pro_state=0;data_state=0;
            if(indev_reset_check(&i->proc)) return;
        }
