...
extern unsigned char TEM_UP_show[6][4];
extern unsigned char TEM_DOWN_show[5][4];
extern volatile unsigned char TEM_UP_arry[6],TEM_DOWN_arry[5];
...
//	  display_sanjiao(1,119,0,1);
		...
		if(F_trip_sy==1)	
   {
    ...
    if(F_trip_sy==0)	array_tmp[0]=0x00;
...
//	Lcd12864_Write16CnCHAR(0,0,0,"5自动试合闸\r6过电压保护\r7欠电压保护\r8断相保护");
//	display_graphic_8x16(1,94,maohao);
//	display_graphic_8x16(3,94,maohao);
//	display_graphic_8x16(5,94,maohao);
//	display_graphic_8x16(7,94,maohao);
...
////	  display_sanjiao(1,119,0,1);
//	  display_kaiguan(1,100,shangdian);
//	  display_kaiguan(3,100,F_trip_gy);	
//	  display_kaiguan(5,100,F_trip_qy);
//	  display_kaiguan_frame(7,100,F_trip_QX);
//	
...
//   clear_screen();
//	  unsigned char array_tmp[3];
//	  Lcd12864_Write16CnCHAR(0,0,0,"13温度保护\r14断零保护");
//		display_graphic_8x16(1,94,maohao);
//		display_graphic_8x16(3,94,maohao);

//		display_kaiguan(1,100,jiaozhun);
//	  display_kaiguan(3,100,F_trip_QL);
//		
//	
//	  if(key_value_tmp==0x10)
//		{
...
//	  array_tmp[0]=ctrlword[2];
//    MODIFY_EEP_W(Addr_crtl_word3,array_tmp,1,0xA0);
//	  clear_define_screen(3,5,100,128);
//		display_kaiguan_frame(3,100,F_trip_QL);
}
...
////	  display_sanjiao(1,119,0,1);
//	  display_kaiguan_frame(1,100,shangdian);
//	  display_kaiguan(3,100,F_trip_gy);	
//	  display_kaiguan(5,100,F_trip_qy);
//	  display_kaiguan(7,100,F_trip_QX);
//	
...
//	  clear_define_screen(0,2,100,120);
//		display_kaiguan_frame(1,100,shangdian);
//}
...
void Temperature_UP()
{
   ...
   unsigned char ARR[2],array_tmp[2];
	EEPROM_R(Addr_temperature_UP,ARR,1,0xA0);
	TH_TEM_UP=ARR[0];
	EEPROM_R(Addr_temperature_DOWN,ARR,1,0xA0);
	TH_TEM_DOWN=ARR[0];
	Lcd12864_Write16CnCHAR(0,0,0,"17高温分闸\r18低温合闸");
	 display_ASCIIstring_8x16(1,96,TEM_UP_show[TH_TEM_UP]);
	 display_ASCIIstring_8x16(3,96,TEM_DOWN_show[TH_TEM_DOWN]);
		display_graphic_8x16(1,88,maohao);
		display_graphic_8x16(3,88,maohao);
	
	
	  if(F_queren==1)
	   {
	     key_state=key_state|TH_TEM_UP;
	   }
		 if((key_value_tmp==0x20)&&(F_queren<3))
	   {
		   F_queren++;
	  ...
	  display_ASCIIstring_8x16_fanxian(1,96,TEM_UP_show[(key_state&0xFF)]);
		 
		 	  if((key_value_tmp==0x20)&&(F_queren==3))
	    {
		    TH_TEM_UP=((key_state)&0x0F);
				TEM_UP=TEM_UP_arry[TH_TEM_UP];
		    array_tmp[0]=TH_TEM_UP ;
		    MODIFY_EEP_W(Addr_temperature_UP,array_tmp,1,0xA0);
		    clear_define_screen(0,2,96,128);
	      display_ASCIIstring_8x16_frame(1,96,TEM_UP_show[TH_TEM_UP]);
        key_state=0x51100;  
        F_queren=1;
			}
   ...
   unsigned char ARR[2],array_tmp[2];
	...
	Lcd12864_Write16CnCHAR(0,0,0,"17高温分闸\r18低温合闸");
	 ...
	 display_ASCIIstring_8x16(3,96,TEM_DOWN_show[TH_TEM_DOWN]);
	  ...
	  if(F_queren==1)
	   {
	     key_state=key_state|TH_TEM_DOWN;
	   }
		 if((key_value_tmp==0x20)&&(F_queren<3))
	   {
		   F_queren++;
	   }
	  clear_define_screen(2,4,96,128);
	  display_ASCIIstring_8x16_fanxian(3,96,TEM_DOWN_show[(key_state&0xFF)]);
		 
		 	if((key_value_tmp==0x20)&&(F_queren==3))
	    {
		    TH_TEM_DOWN=((key_state)&0x0F);
				TEM_DOWN=TEM_DOWN_arry[TH_TEM_DOWN];
		    array_tmp[0]=TH_TEM_DOWN ;
		    MODIFY_EEP_W(Addr_temperature_DOWN,array_tmp,1,0xA0);
		    clear_define_screen(2,4,96,128);
	      display_ASCIIstring_8x16_frame(3,96,TEM_DOWN_show[TH_TEM_DOWN]);
...
void Fun_Corr_Slec()
{
    clear_screen();
	   unsigned char array_tmp[3];
	  Lcd12864_Write16CnCHAR(0,0,0,"17电流合闸\r18温度保护\r19通信方式");
	  ...
	  DL_hezha=array_tmp[0];
		...
		display_graphic_8x16(3,94,maohao);
	  display_graphic_8x16(5,94,maohao);
	  display_kaiguan(1,100,DL_hezha);
		display_kaiguan(3,100,wendu);
	   display_kaiguan(5,100,F_TX);
	  EEPROM_R(Addr_wendu,array_tmp,1,0xA0);
	  wendu=array_tmp[0];
	  EEPROM_R(Addr_TX,array_tmp,1,0xA0);
	  ...
	  if(key_value_tmp==0x10)
        ...
        if(wendu==1)	
				{
					array_tmp[0] = 0x01;//T
					clear_define_screen(3,4,100,128);
		    	display_kaiguan_frame(3,100,1);
					F_Corr=1;
				}
        else if(wendu==0)
				{
...
void Fun_TX_switch()
{
	 clear_screen();
	  ...
	  if(key_value_tmp==0x10)
		{
			F_TX=1;
			 MX_USART3_UART_Init(); ///9600,Modbus
		}
		if(key_value_tmp==0x40)
		{
			F_TX=0;
//			 MX_USART3_UART_Init1(); 
		}
		 if(F_TX==1)	array_tmp[0] = 0x01;
     if(F_TX==0)	array_tmp[0] = 0x00;
        MODIFY_EEP_W(Addr_TX,array_tmp,1,0xA0);
		clear_define_screen(5,6,100,128); 
		display_kaiguan_frame(5,100,F_TX);
}
